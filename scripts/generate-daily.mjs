...
import { cleanArticleSummary, isReadableArticleCandidate } from './article-eligibility.mjs';
...
const repairNonArticle = process.argv.includes('--repair-non-article');
...
function validCandidate(item){
  try{
    ...
    return isReadableArticleCandidate(item)&&item.title.length>12&&item.summary.length>60&&url.hostname.endsWith(item.source.host)&&Number.isFinite(age)&&age>=-2&&age<=30;
  }catch{return false}
}
...
const existingEdition=editions.find(edition=>edition.date===date);
if(existingEdition&&(!repairNonArticle||isReadableArticleCandidate(existingEdition.works.find(work=>work.type==='article')||{}))){
...
const modern={
  ...
  text:cleanArticleSummary(article.summary),tags:['contemporary',article.category.toLowerCase()].filter(Boolean),difficulty:'Moderate',
  rationale:'Recent, source-verified, and selected with a penalty against repeatedly using the same publication.'
};
const works=existingEdition?existingEdition.works.map(work=>work.type==='article'?modern:work):[modern,chooseClassic('poem'),chooseClassic('story')];
const next=existingEdition?{
  ...existingEdition,
...
console.log(`✓ ${existingEdition?'Repaired':'Created'} edition ${date}: ${works.map(work=>work.title).join(' · ')}`);
