...
const totalMinutes = edition => edition.works.reduce((n,x)=>n+x.minutes,0);
...
function workCard(work){
  ...
  return `<article class="work" id="${work.id}">
    ...
    <header class="work-header"><div class="eyebrow">${esc(work.category)} · ${work.year||'Year unknown'}</div><h2>${esc(work.title)}</h2>
      ...
      <p class="work-facts">${esc(work.author.culture)} · ${work.century} · ${work.minutes} min read</p><p class="introduction">${esc(work.intro)}</p></header>
...
function todayPage(edition=todayEdition){
  return `<section class="edition-hero"><div class="eyebrow">${formatDate(edition.date)} · Edition No. ${String(edition.number).padStart(3,'0')}</div><p class="edition-schedule">New edition scheduled daily at 1:17 p.m. Eastern</p><h1>Tonight’s<br>Reading</h1><p class="dek">${esc(edition.curatorNote)}</p><div class="edition-meta"><span>${totalMinutes(edition)} minutes</span><span>Three works · ${esc(edition.theme)}</span><span data-edition-progress>${state().read.filter(id=>edition.works.some(w=>w.id===id)).length} of ${edition.works.length} read</span></div></section>
  <section class="edition-overview" aria-label="Tonight's table of contents">${edition.works.map(w=>`<button class="overview-item text-button" data-scroll="${w.id}"><span class="eyebrow">${workType(w.type)}</span><h2>${esc(w.title)}</h2><p>${esc(w.author.name)} · ${w.minutes} min</p></button>`).join('')}</section>
...
function archivePage(){ return pageHero('The Archive','Seven evenings of stories, poems, and ideas—kept here to return to.')+`<div class="content-shell"><div class="archive-grid">${editions.map(e=>`<article class="archive-row" tabindex="0" data-edition="${e.id}"><div><span class="small-label">${e.date.slice(5).replace('-',' / ')}</span><p>${e.date.slice(0,4)}</p></div><div class="edition-col small-label">No. ${String(e.number).padStart(3,'0')}</div><div><h2>${esc(e.theme)}</h2><p>${e.works.map(w=>w.author.name).join(' · ')}</p></div><div class="time-col small-label">${totalMinutes(e)} min</div></article>`).join('')}</div></div>` }
