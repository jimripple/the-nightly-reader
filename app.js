...
function favoritesPage(){ const favs=allWorks.filter(w=>state().favorites.includes(w.id)); return pageHero('Favorites','The pieces you want to keep close.')+`<div class="content-shell">${favs.length?favs.map(w=>`<article class="result"><div class="small-label">${workType(w.type)}</div><div><div class="small-label favorite-date">Featured ${formatDate(w.editionDate)} · Edition No. ${String(w.editionNumber).padStart(3,'0')}</div><h2>${esc(w.title)}</h2><p>${esc(w.author.name)} · ${esc(w.intro)}</p></div><button class="action" data-open-work="${w.editionId}|${w.id}">Read</button></article>`).join(''):empty('Nothing saved yet','Use “Favorite” beneath any reading to place it here.')}</div>` }
