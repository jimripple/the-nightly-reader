...
function todayPage(edition=todayEdition){
  return `<section class="edition-hero"><div class="eyebrow">${formatDate(edition.date)} · Edition No. ${String(edition.number).padStart(3,'0')}</div><h1>Tonight’s<br>Reading</h1><p class="dek">${esc(edition.curatorNote)}</p><div class="edition-meta"><span>${totalMinutes(edition)} minutes</span><span>Three works · ${esc(edition.theme)}</span><span data-edition-progress>${state().read.filter(id=>edition.works.some(w=>w.id===id)).length} of ${edition.works.length} read</span></div></section>
