...
function todayPage(edition=todayEdition){
  ...
  return `<section class="edition-hero"><div class="edition-dateline eyebrow"><span>${dateLabel.slice(0,comma)}</span><span>${dateLabel.slice(comma+2)}</span><span>Edition No. ${String(edition.number).padStart(3,'0')}</span></div><p class="edition-schedule">New edition scheduled daily at 1:17 p.m. Eastern</p><h1>Tonight’s<br>Reading</h1><p class="dek">${esc(edition.curatorNote)}</p><div class="edition-meta"><span>Three works · ${esc(edition.theme)}</span><span data-edition-progress>${state().read.filter(id=>edition.works.some(w=>w.id===id)).length} of ${edition.works.length} read</span></div></section>
