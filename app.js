...
function workCard(work){
  ...
  return `<article class="work" id="${work.id}" data-work-type="${work.type}">
    ...
    <div class="reader">${external?`<div class="external-reading"><div class="eyebrow">From ${esc(work.source)}</div><a class="source-cta" data-external-source href="${work.sourceUrl}" target="_blank" rel="noopener">Read the complete article at ${esc(work.source)} ↗</a><span>Your place here will be saved.</span></div>`:`<p class="reader-text" data-work-id="${work.id}" ${!fullTexts[contentId]&&remoteTextPages[contentId]?`data-full-page="${esc(remoteTextPages[contentId])}"`:''}>${esc(fullTexts[contentId] || work.text)}</p><p class="source-note" data-source-note="${work.id}">${fullTexts[contentId]?'Complete public-domain text.':'Loading complete public-domain text… Source:'} <a href="${work.sourceUrl}" target="_blank" rel="noopener">${esc(work.source)} ↗</a>. The source link is provided for provenance; you can read the entire piece here.</p>`}
