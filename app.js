...
const state = () => ({ read:[], favorites:[], notes:{}, ...store.read() });
...
function hydrateFullTexts(){
  document.querySelectorAll('[data-full-page]').forEach(async element=>{
    ...
    try{ const raw=await loadWikisourceText(element.dataset.fullPage); const title=element.closest('.work')?.querySelector('.work-header h2')?.textContent||''; const text=cleanSourceText(raw,title); if(text.split(/\s+/).length<80) throw new Error('Source text was incomplete'); element.textContent=text; element.removeAttribute('data-full-page'); if(note)note.firstChild.textContent='Complete public-domain text. Source: '; }
...
function workCard(work){
  ...
  return `<article class="work" id="${work.id}" data-work-type="${work.type}">
    <div class="work-rule"><span>${workType(work.type)}</span></div>
    <header class="work-header"><div class="eyebrow">${esc(external ? work.source : work.category)} · ${work.year||'Year unknown'}</div><h2>${esc(work.title)}</h2>
      <div class="byline">by ${esc(work.author.name)} ${external?'':`<span>${esc(work.author.lifespan)}</span>`}</div>
      ${external?'':`<p class="work-facts">${esc(work.author.culture)} · ${work.century}</p>`}<p class="introduction">${esc(external ? work.text || work.intro : work.intro)}</p></header>
    <div class="reader">${external?'':`<p class="reader-text" data-work-id="${work.id}" ${!fullTexts[contentId]&&remoteTextPages[contentId]?`data-full-page="${esc(remoteTextPages[contentId])}"`:''}>${esc(fullTexts[contentId] || work.text)}</p>`}</div>
    <footer class="reading-finish"><div class="reading-source"><div class="eyebrow">${external?'Continue reading':'About this text'}</div>${external?`<h3>The full article lives at ${esc(work.source)}.</h3><a class="source-cta" data-external-source href="${work.sourceUrl}" target="_blank" rel="noopener">Read at ${esc(work.source)} <span aria-hidden="true">↗</span></a><p>Your place here will be saved when you return.</p>`:`<p class="source-note" data-source-note="${work.id}">${fullTexts[contentId]?'Complete public-domain text.':'Loading complete public-domain text… Source:'} <a href="${work.sourceUrl}" target="_blank" rel="noopener">${esc(work.source)} ↗</a>. The source link is provided for provenance; you can read the entire piece here.</p>`}<div class="tags">${work.tags.map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</div></div>
      <div class="reading-personal"><div class="eyebrow">Your reading</div><h3>Keep this one close.</h3><div class="actions"><button class="action ${isRead?'active':''}" data-action="read" data-id="${work.id}">${isRead?'✓ Read':'Mark as read'}</button><button class="action ${isFav?'active':''}" data-action="favorite" data-id="${work.id}">${isFav?'♥ Saved':'♡ Favorite'}</button></div>
      <details class="reflection"><summary>Leave a private note <span class="reflection-optional">Optional reflection</span></summary><div class="reflection-body"><div class="reflection-inner"><div class="prompts"><p>What stayed with you? What did you disagree with? What idea do you want to remember?</p></div><textarea class="note" data-note="${work.id}" placeholder="Write a private note…">${esc(s.notes[work.id]||'')}</textarea><p class="privacy-note">Private to this browser. Your note is never sent to The Nightly Reader or anyone else.</p></div></div></details></div>
    </footer></article>`;
}
...
function favoritesPage(){ const favs=[...allWorks,...replacedWorks].filter(w=>state().favorites.includes(w.id)); return pageHero('Favorites','The pieces you want to keep close.')+`<div class="content-shell">${favs.length?favs.map(w=>`<article class="result"><div class="small-label">${workType(w.type)}</div><div><div class="small-label favorite-date">Featured ${formatDate(w.editionDate)} · Edition No. ${String(w.editionNumber).padStart(3,'0')}</div><h2>${esc(w.title)}</h2><p>${esc(w.author.name)} · ${esc(w.intro)}</p></div>${w.editionId?`<button class="action" data-open-work="${w.editionId}|${w.id}">Read</button>`:`<a class="action" href="${esc(w.sourceUrl)}" target="_blank" rel="noopener">Source ↗</a>`}</article>`).join(''):empty('Nothing saved yet','Use “Favorite” beneath any reading to place it here.')}</div>` }
function settingsPage(){ return pageHero('Notes & privacy','Your reading notes belong to you, not to the site.')+`<div class="content-shell settings"><section class="private-data"><div class="eyebrow">Private data</div><h2>Your notes stay on this device</h2><p>The Nightly Reader has no account or notes server. Notes are stored only in this browser, so the site owner cannot read them. Export a readable CSV whenever you like. Download the private backup if you want to restore the notes in another browser later.</p><div class="private-data-actions"><button class="action" id="export-notes-csv">Export notes as CSV</button><button class="action" id="export-notes-backup">Download restorable backup</button><label class="action file-action" for="import-notes">Restore backup</label><input id="import-notes" type="file" accept="application/json,.json" hidden></div></section></div>` }
...
function bind(){
  ...
  const exportCsv=document.querySelector('#export-notes-csv');if(exportCsv)exportCsv.onclick=()=>{const notes=state().notes, quote=value=>`"${String(value??'').replaceAll('"','""')}"`;const rows=[['Edition date','Form','Title','Author','Source link','Note'],...[...allWorks,...replacedWorks].filter(work=>notes[work.id]?.trim()).map(work=>[work.editionDate,workType(work.type),work.title,work.author.name,work.sourceUrl,notes[work.id]])];download(`\uFEFF${rows.map(row=>row.map(quote).join(',')).join('\r\n')}`,'text/csv;charset=utf-8',`nightly-reader-notes-${new Date().toISOString().slice(0,10)}.csv`);toast(`${rows.length-1} notes exported as CSV`)};
...
function applyTheme(theme,persist=false){ document.documentElement.dataset.theme=theme; if(persist)try{localStorage.setItem('nightly-theme',theme)}catch{} document.querySelector('meta[name="theme-color"]').content=theme==='dark'?'#171714':'#f5f1e8'; const button=document.querySelector('#theme-toggle'); const dark=theme==='dark'; button.setAttribute('aria-label',dark?'Use light theme':'Use dark theme'); button.title=dark?'Use light theme':'Use dark theme'; button.querySelector('span').textContent=dark?'☀':'◐'; }
document.querySelector('#theme-toggle').onclick=()=>applyTheme(document.documentElement.dataset.theme==='dark'?'light':'dark',true);
applyTheme(document.documentElement.dataset.theme||'light');
matchMedia('(prefers-color-scheme: dark)').addEventListener('change',event=>{ let saved;try{saved=localStorage.getItem('nightly-theme')}catch{} if(saved!=='dark'&&saved!=='light')applyTheme(event.matches?'dark':'light'); });
